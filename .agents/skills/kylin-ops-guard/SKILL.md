...
Currently spec-only. Eight `*.md` files at repo root are the v0.1 product/architecture/task specs (Chinese). No `backend/` or `frontend/` exists yet. `麒麟安全智能运维 Agent P0 开发启动包 v0.1.md` is empty (0 bytes). Repo-level `AGENTS.md` exists and mirrors the rules below. When scaffolding starts, follow the package/page layouts in [references/architecture.md](references/architecture.md) exactly.
